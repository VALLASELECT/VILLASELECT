      ...
      <section style={{ background:"linear-gradient(135deg,#fff8f3,#fff3e8)", padding:"60px 24px", borderTop:"1px solid #f0e8e0", borderBottom:"1px solid #f0e8e0" }}>
        <div style={{ maxWidth:800, margin:"0 auto", textAlign:"center" }}>
          ...
          <div style={{ display:"flex", justifyContent:"center", gap:24, flexWrap:"wrap" }}>
            {["🔒 Paiement 100% sécurisé","🛡️ Données chiffrées SSL","✅ 3D Secure activé"].map((item,i) => (
    ...
    <div className="card" style={{ borderRadius:16 }}>
      ...
      <div style={{ padding:20 }}>
        ...
        <div style={{ display:"flex", gap:8, flexWrap:"wrap", marginBottom:16 }}>
          ...
          {villa.ac && <span className="badge" style={{ background:"#e0f7fa", color:"#00838f" }}>❄️ Clim</span>}
...
function LocationsPage({ villas, reviews, searchFilters, nav, currentUser, showToast, reservations, setReservations, saveReservationToDB }) {
  ...
  const [payStep, setPayStep] = useState("details"); // details | card | processing | success
    ...
    <div style={{ maxWidth:1200, margin:"0 auto", padding:"40px 24px" }}>
      ...
      <div style={{ display:"grid", gridTemplateColumns:"280px 1fr", gap:28, alignItems:"start" }}>
        ...
        <div style={{ background:"#fff", borderRadius:16, padding:24, boxShadow:"0 2px 16px rgba(0,0,0,.06)", position:"sticky", top:80 }}>
          ...
          {[["pool","🏊 Piscine"],["ac","❄️ Climatisation"],["wifi","📶 Wifi"],["parking","🚗 Parking"]].map(([key, label]) => (
      ...
      <Modal open={!!bookVilla} onClose={() => { setBookVilla(null); setPayStep("details"); }} title={
        ...
        payStep === "card" ? "💳 Paiement sécurisé" :
        payStep === "processing" ? "Traitement en cours..." : "✅ Réservation confirmée !"
      }>
        {bookVilla && (
          <div>
              ...
              <div>
                ...
                {nights > 0 && nights < 7 && (
                  <div style={{ background:"#fde8e8", color:"#c53030", padding:"10px 14px", borderRadius:10, fontSize:13, marginBottom:12 }}>
                    ⚠️ Durée minimale : 7 nuits ({nights} nuit{nights>1?"s":""} sélectionnée{nights>1?"s":""})
                  </div>
                )}
                {nights >= 7 && (
              ...
              <div>
                ...
                <div style={{ display:"flex", alignItems:"center", gap:8, marginBottom:20 }}>
                  ...
                  <div style={{ display:"flex", alignItems:"center", gap:6 }}>
                    ...
                    <span style={{ fontSize:12, fontWeight:600, color:"#F28C38" }}>Paiement</span>
                  </div>
                  <div style={{ flex:1, height:2, background:"#e2e8f0" }} />
                  <div style={{ display:"flex", alignItems:"center", gap:6 }}>
                    <div style={{ width:28, height:28, borderRadius:"50%", background:"#e2e8f0", color:"#aaa", display:"flex", alignItems:"center", justifyContent:"center", fontSize:13, fontWeight:700 }}>3</div>
                ...
                <div style={{ background:"linear-gradient(135deg,#5A2E0C,#8B4513)", borderRadius:14, padding:20, marginBottom:20, color:"#fff" }}>
                  <p style={{ fontSize:13, opacity:.8, marginBottom:4 }}>Acompte à payer maintenant (50%)</p>
                  <p style={{ fontFamily:"Montserrat,sans-serif", fontWeight:900, fontSize:32, marginBottom:8 }}>{formatPrice(deposit)}</p>
                  ...
                  <p style={{ fontSize:12, opacity:.7, marginTop:4 }}>
                    📅 {new Date(bookForm.checkIn).toLocaleDateString("fr-FR")} → {new Date(bookForm.checkOut).toLocaleDateString("fr-FR")}
                  </p>
                </div>
                ...
                <div style={{ display:"flex", justifyContent:"center", gap:10, marginBottom:20 }}>
                  <CardLogos.Visa h={28} />
                  <CardLogos.Mastercard h={28} />
                  <CardLogos.Amex h={28} />
                  <CardLogos.CarteBleue h={28} />
                </div>

                {/* Bouton Stripe */}
                <button style={{ width:"100%", background:"linear-gradient(135deg,#635bff,#4f46e5)", color:"#fff", border:"none", padding:"18px", borderRadius:14, fontSize:17, fontWeight:700, cursor:"pointer", marginBottom:12, display:"flex", alignItems:"center", justifyContent:"center", gap:10 }} onClick={submitPayment}>
                  🔒 Payer {formatPrice(deposit)}
                </button>

                <div style={{ background:"#f0f9ff", border:"1px solid #bae6fd", borderRadius:10, padding:12, marginBottom:12, fontSize:12, color:"#0369a1", textAlign:"center" }}>
                  Vous serez redirigé vers la page de paiement sécurisée Stripe.<br/>
            ...
            {payStep === "redirecting" && (
              <div style={{ textAlign:"center", padding:"40px 20px" }}>
                ...
                <h3 style={{ fontFamily:"Montserrat,sans-serif", color:"#5A2E0C", marginBottom:12 }}>Redirection vers Stripe...</h3>
                <p style={{ color:"#555", fontSize:14, lineHeight:1.8, marginBottom:20 }}>
                  Vous allez être redirigé vers la page de paiement sécurisée Stripe.<br/>
                  <strong>Votre réservation sera confirmée uniquement après paiement.</strong>
                </p>
                <div style={{ display:"flex", justifyContent:"center", gap:6, marginTop:20 }}>
                  {[0,1,2].map(i => (
                    <div key={i} style={{ width:10, height:10, borderRadius:"50%", background:"#F28C38", animation:`bounce 0.8s ${i*0.2}s infinite alternate` }} />
                  ))}
                </div>
            ...
            {/* ── ÉTAPE 4 : Succès après retour Stripe ── */}
            {payStep === "success" && (
              <div style={{ textAlign:"center", padding:"20px 0" }}>
                ...
                <h3 style={{ fontFamily:"Montserrat,sans-serif", color:"#276749", fontSize:22, marginBottom:8 }}>Paiement confirmé !</h3>
                <p style={{ color:"#555", fontSize:14, marginBottom:20 }}>
                  Votre acompte de <strong style={{ color:"#F28C38" }}>{formatPrice(deposit)}</strong> a été reçu.<br/>
                ...
                <div style={{ background:"#f8fafc", borderRadius:12, padding:16, marginBottom:20, textAlign:"left" }}>
                  <p style={{ fontSize:13, lineHeight:2, color:"#555" }}>
                    ...
                    💰 Total séjour : {formatPrice(total)} · Reste à payer : {formatPrice(deposit)}
                  </p>
                </div>
    ...
    <div style={{ position:"fixed", inset:0, background:"#fff", zIndex:1000, overflowY:"auto", padding:"20px 16px" }}>
      ...
      <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:24, paddingBottom:16, borderBottom:"2px solid #f0e8e0" }}>
        <h2 style={{ fontFamily:"Montserrat,sans-serif", color:"#5A2E0C", fontSize:22, fontWeight:800 }}>
          {form.id ? "✏️ Modifier la villa" : "➕ Nouvelle villa"}
        </h2>
      ...
      {/* Équipements */}
      <label style={labelStyle}>⚙️ Équipements</label>
      <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:10, marginBottom:16 }}>
        {[["wifi","📶 Wifi"],["ac","❄️ Climatisation"],["pool","🏊 Piscine"],["parking","🚗 Parking"]].map(([key, label]) => (
